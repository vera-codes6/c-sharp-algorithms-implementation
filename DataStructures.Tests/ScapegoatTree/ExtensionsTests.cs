...
public class ExtensionsTests
{
    ...
    public void RebuildFlatTree_ValidFlatTree_RebuildsTree()
    {
        ...
        {
            Left = new Node<int>(1) { Left = new Node<int>(-1), Right = new Node<int>(2) },
            Right = new Node<int>(6) { Left = new Node<int>(5) },
        };

        var list = new List<Node<int>> { new(-1), new(1), new(2), new(3), new(5), new(6) };
    ...
    public void RebuildFromList_RangeIsInvalid_ThrowsException()
    {
        Assert.Throws<ArgumentException>(() =>
            Extensions.RebuildFromList(new List<Node<int>>(), 1, 0)
        );
    }
}
