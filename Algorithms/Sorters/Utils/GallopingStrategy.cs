...
public static class GallopingStrategy<T>
{
    public static int GallopLeft(T[] array, T key, int baseIndex, int length, IComparer<T> comparer)
    {
        ...
        var (offset, lastOfs) =
            comparer.Compare(key, array[baseIndex]) > 0
    ...
    public static int BoundLeftShift(int shiftable) =>
        (shiftable << 1) < 0 ? (shiftable << 1) + 1 : int.MaxValue;
    ...
    private static (int Offset, int LastOfs) RightRun(
        T[] array,
        T key,
        int baseIndex,
        int len,
    ...
    private static int FinalOffset(
        T[] array,
        T key,
        int baseIndex,
        int offset,
